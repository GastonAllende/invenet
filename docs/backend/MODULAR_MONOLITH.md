...
│   │           └── ModularDbContext.cs
│   │
│   ├── <FeatureModule>/              # Example generic feature module
│   │   ├── <Feature>Module.cs       # Module registration
...
│   │       └── Data/
│   │           └── <Feature>EntityConfiguration.cs
...
Individual business capabilities are implemented as distinct modules. Each follows a structured vertical slice:

- **API**: REST controllers and route definitions
- **Domain**: Core entities and business rules (no external dependencies)
- **Features**: Commands, queries, and cohesive feature boundaries
- **Infrastructure**: Data access, EF Core configurations, and external API integrations

The exact modules available in the system will scale dynamically over time. Discover active modules in `apps/api/Invenet.Api/Modules/`.
